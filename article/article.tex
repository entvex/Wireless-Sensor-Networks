...
we build a relaying protocol based on signal strength and accumulated transmission errors (lost packets) and use it to investigate what factors it pays for a WSN to relay by: The signal quality (how many times did we notice an event), energy consumption aiming to use the least amount of energy to extend lifetime, or conceivably a comprimise of both. Events in our case is the heart rate of a athlete runner running a oval-formed running track. We find that creating such a multivariate decision protocol is indeed possible and our results show XXX
